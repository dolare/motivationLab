    ...
    function borntogive_maintenance_mode() {
        ...
        if ($maintenance_mode) {
            if (!current_user_can('edit_themes') || !is_user_logged_in()) {
                wp_die($custom_logo_output . '<p style="text-align:center">' . __('We are currently in maintenance mode, please check back shortly.', 'borntogive') . '</p>', __('Maintenance Mode', 'borntogive'));
            }
        }
    }
...
@media only screen and (max-width: 992px) {
	.dd-menu > li:hover > a, .dd-menu > li:hover > a > i{
		...
		if ($mmbackground != '')
		{
			echo '@media only screen and (max-width: 992px) {.dd-menu{background:'.esc_attr($mmbackground).';}}';
		}
		
		if (isset($options['mobile_menu_color'])&&isset($options['mobile_menu_color']['regular'])&&$options['mobile_menu_color']['regular'] != '') {
			echo '@media only screen and (max-width: 992px) {.dd-menu > li > a{color:'.$options['mobile_menu_color']['regular'].';}}';
		}
		if (isset($options['mobile_menu_color'])&&isset($options['mobile_menu_color']['hover'])&&$options['mobile_menu_color']['hover'] != '') {
			echo '@media only screen and (max-width: 992px) {.dd-menu > li > a:hover{color:'.$options['mobile_menu_color']['hover'].';}}';
		}
		if (isset($options['mobile_menu_color'])&&isset($options['mobile_menu_color']['active'])&&$options['mobile_menu_color']['active'] != '') {
			echo '@media only screen and (max-width: 992px) {.dd-menu > li > a:active{color:'.$options['mobile_menu_color']['active'].';}}';
		}
		if (isset($options['mobile_menu_border']['border-bottom'])&&$options['mobile_menu_border']['border-bottom'] != '') {
			echo '@media only screen and (max-width: 992px) {.dd-menu > li > a{border-bottom-width:'.$options['mobile_menu_border']['border-bottom'].';border-bottom-style:'.$options['mobile_menu_border']['border-style'].';border-bottom-color:'.$options['mobile_menu_border']['border-color'].';}}';
		}
...
if (!function_exists('borntogive_register_sidebars')) {
    ...
    add_action('init', 'borntogive_register_sidebars', 35);
}
/* -------------------------------------------------------------------------------------
...
if (!function_exists('borntogive_register_meta_box')) {
    add_action('admin_init', 'borntogive_register_meta_box');
    function borntogive_register_meta_box() {
        ...
        $meta_box = array(
            ...
            'title' => esc_html__("Select Sidebar", 'borntogive'),
            'pages' => array('post', 'page', 'event', 'team', 'campaign'),
            'context' => 'normal',
            'fields' => array(
                array(
                    ...
                    'type' => 'select',
                    'options' => borntogive_get_all_sidebars(),
                ),
                array(
	...
	if($post_type=='event')
	{
		...
		if($event_frequency_type==1)
		{
			if($event_recur_count>0)
			{
				...
				update_post_meta($postId,'borntogive_event_frequency_end',date('Y-m-d', $update_date));
			}
			else
			{
		...
		elseif($event_multiple_type==1)
		{
			...
			update_post_meta($postId,'borntogive_event_frequency_end',date('Y-m-d', $mostRecent));
		}
		else
		{
			update_post_meta($postId,'borntogive_event_frequency_end',date('Y-m-d', $event_end_date_unix));
		}
...
if(mail($manager_email, $e_subject, $msg, $headers)&&mail($email, $e_subject, $user_msg, $user_headers)) {
	//echo "<div class=\"alert alert-success\"><p>".esc_html__("Thank you", "borntogive")." <strong>$name</strong>, ".esc_html__("your message has been submitted to us.", "borntogive")."</p>";
	if($post_type=="event")
	{
...
global $options, $woocommerce;
			if(class_exists('Woocommerce')):
				...
				<div class="cart-module">
						<a href="#" class="cart-module-trigger" id="cart-module-trigger"><i class="fa fa-shopping-cart"></i><span class="cart-tquant"><?php echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'borntogive'), $woocommerce->cart->cart_contents_count);?></span></a>
						<div class="cart-module-opened">
							...
							<div class="cart-module-footer">
								<a href="<?php echo get_permalink(get_option('woocommerce_cart_page_id')); ?>" class="basic-link"><i class="fa fa-long-arrow-left"></i> <?php _e('View cart', 'borntogive'); ?></a>
								<a href="<?php echo get_permalink(get_option('woocommerce_checkout_page_id')); ?>" class="basic-link"><?php _e('Checkout', 'borntogive'); ?> <i class="fa fa-long-arrow-right"></i></a>
							</div>
						</div>
					</div>
				<?php endif; ?>
